...
```bash
~/.claude/skills/agent-friendly-docs-pro-max/scripts/arm-watcher.sh "<your-tree>" --install
...
```bash
python3 ~/.claude/skills/agent-friendly-docs-pro-max/scripts/graph.py "<your-tree>"
...
Full install (both tiers) is in the **[monorepo README](../README.md)**. In short: clone the repo, then
symlink this folder as the skill —

```bash
ln -s <repo>/pro-max ~/.claude/skills/agent-friendly-docs-pro-max
```

Invoke `/agent-friendly-docs-pro-max`, or just ask in natural language (English or Portuguese):
